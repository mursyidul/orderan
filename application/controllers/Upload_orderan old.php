...
class Upload_orderan extends CI_Controller {
    ...
    public function index(){
        ...
        foreach ($list_order as $k) {
            ...
            for ($a=0; $a < $jumlah_order[0]['jumlah']; $a++) { 
                $nomor_sku        = $no_pesanan_order[$a]['nomor_sku'];
                    // echo "nomor_sku : ".$nomor_sku;
                if ($nomor_sku != "") {
                    $sku_order  = $this->Model_orderan->get_nomor_sku($nomor_sku);
                    ...
                    array_push($nomor_sku_order, array(
                        ...
                        'gambar'  => '',
                        'deskripsi'  => '',
                        'nama_variasi'  => $no_pesanan_order[$a]['nama_variasi'],
                        'nama_produk'  => $no_pesanan_order[$a]['nama_produk'],
                        'total_qty'  => $no_pesanan_order[$a]['total_qty'],
                    ));
                }
        ...
        // $data["orderan"]  = $this->Model_orderan->get_upload_orderan();
        // echo $this->db->last_query();
    ...
    public function change_status_cetak_selesai(){
        $no_pesanan   = $this->input->post("no_pesanan");
        $created_date = date("Y-m-d H:i:s");
        $action = $this->Model_orderan->change_status_cetak_selesai($no_pesanan, $created_date);
        if ($action) {
    ...
    public function change_status_batal_orderan(){
        $no_pesanan = $this->input->post('no_pesanan');
        $action = $this->Model_orderan->change_status_batal_orderan($no_pesanan);
        if ($action) {
