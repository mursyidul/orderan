...
class Orderan_batal extends CI_Controller {
    ...
    public function index(){
        ...
        $list_order = $this->Model_orderan->get_orderan_batal();
        ...
        foreach ($list_order as $k) {
            ...
            $id_user          = $k['id_user'];
            $jumlah_order     = $this->Model_orderan->get_jumlah_orderan_batal($no_pesanan);
            $no_pesanan_order = $this->Model_orderan->get_no_pesanan_batal($no_pesanan);
            ...
            for ($a=0; $a < $jumlah_order[0]['jumlah']; $a++) { 
                ...
                if ($nomor_sku != "") {
                    // echo "tidak kosong";
                    $sku_order  = $this->Model_orderan->get_nomor_sku_batal($sku_induk, $id_orderan);
                    // echo $this->db->last_query();
		...
		$this->template->load("template", "orderan/data-orderan_batal", $data);
    }
}
?>